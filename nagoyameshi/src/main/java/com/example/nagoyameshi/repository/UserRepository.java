...
import com.example.nagoyameshi.repository.Projection.UserSubscriptionProjection;
	...
	@Query(value = """
			...
			""", countQuery = """
			    ...
			    WHERE
					(:keyword IS NULL OR u.name LIKE CONCAT('%', :keyword, '%') OR u.email LIKE CONCAT('%', :keyword, '%'))
			""", nativeQuery = true)
	public Page<UserSubscriptionProjection> searchUserWithLatestSubscriptions(@Param("keyword") String keyword,
			Pageable pageable);
